...
interface IJiraIssueParameters {
  issueKey: string;
}
  ...
  implements vscode.LanguageModelTool<IJiraIssueParameters>
{
  private getJiraConfig(): JiraConfig | null {
    const config = vscode.workspace.getConfiguration("jiraChatTool");
            ...
            JSON.stringify(
              {
                ...
                requiredSettings: [
                  "jiraChatTool.baseUrl",
                  "jiraChatTool.email",
                  "jiraChatTool.apiToken",
                ],
                helpUrl:
...
export function registerJiraTools(context: vscode.ExtensionContext) {
